...
class FindDuplicates:
    ...
    def make_wide(self):
        try:
            self._duples_wide_data = pd.merge(self.duples_data,
                                              ...
                                              on=self.data.obj_id_col)
            self._duples_wide_data = pd.merge(self._duples_wide_data,
                                              self.data.data_norm[self.result_cols],
                                              left_on='duplicate_id', right_on=self.data.obj_id_col,
                                              suffixes=('-src', '-dupl')) \
                .rename(columns={self.data.obj_id_col + '-src': self.data.obj_id_col}) \
                .drop([self.data.obj_id_col + '-dupl'], axis=1)
    ...
    def make_long(self):
        try:
            ...
            df_long.loc[df_long['source'] == 'dupl', self.data.obj_id_col] = df_long['duplicate_id']
            df_long.drop(["source", "duplicate_id"], axis=1, inplace=True)
            df_long.drop_duplicates(subset=[self.data.obj_id_col, 'duplicates_cluster'],
                                    keep='first',
                                    inplace=True)
    ...
    def collect_duplicates_clusters(self, obj_id, cluster, df):
        res = df[df[self.data.obj_id_col] == obj_id][[self.data.obj_id_col, 'duplicate_id']]
    ...
    def cluster_duplicates(self):
        ...
        clusters = []
        for row in df_pairs.sort_values(by=self.data.obj_id_col).itertuples(index=False):
        ...
        for i, cl in enumerate(clusters):
            df_pairs.loc[df_pairs[self.data.obj_id_col].isin(cl), 'duplicates_cluster'] = i
    ...
    def extract_pairs(self):
        if 'duplicates' in self.data.data_norm.columns:
            df_duples = self.data.data_norm[[self.data.obj_id_col, 'duplicates']].copy()
            df_duples.fillna('', inplace=True)
            duplicates_list = list()
            for item in df_duples[[self.data.obj_id_col, 'duplicates']].itertuples(index=False):
                item_matches = []
                duples_list = item[1] if type(item[1]) == list else eval(item[1])
                if len(duples_list) > 0:
                    for row in duples_list:  # eval
                        dupl_item = {self.data.obj_id_col: row['source_id'],
            ...
            return pd.DataFrame(duplicates_list)[[self.data.obj_id_col, 'duplicate_id']]
        else:
